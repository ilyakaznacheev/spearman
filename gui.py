...
class Window(tk.Tk):
    ...
    def _init_widgets(self):
        self.pan = tk.PanedWindow()

        self.pan.pack(fill=tk.BOTH, expand=1)
    ...
    def _init_menu(self, frame):
        frame.widgets["label_host"] = tk.Label(self.menu_frame, text="host")
        frame.widgets["entry_host"] = tk.Entry(self.menu_frame)

        frame.widgets["label_port"] = tk.Label(self.menu_frame, text="port")
        frame.widgets["entry_port"] = tk.Entry(self.menu_frame)

        frame.widgets["label_frame"] = tk.Label(self.menu_frame, text="frame size")
        frame.widgets["entry_frame"] = tk.Entry(self.menu_frame)

        frame.widgets["button_start"] = tk.Button(self.menu_frame, text="start")
        frame.widgets["button_start"].bind("<Button-1>", self.event_start)

        frame.widgets["button_stop"] = tk.Button(self.menu_frame, text="stop")
    ...
    def _init_canvas(self, frame, size=CANVAS_SIZE, shift=CANVAS_INDENT):
        ...
        # self.renew_colors({(1, 2): 0.123, (3, 4): 0.789})
